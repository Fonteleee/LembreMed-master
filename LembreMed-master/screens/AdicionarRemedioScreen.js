...
export default function AdicionarRemedioScreen({ navigation }) {
  ...
  const salvar = async () => {
    ...
    RemedioService.adicionarRemedio({ nome, horarios, frequencia });
  ...
  return (
    <View style={[styles.container, { backgroundColor: scheme === 'dark' ? '#222' : '#fff' }] }>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity onPress={() => setFrequencia(item.value)} style={{ margin: 5, padding: 8, backgroundColor: frequencia === item.value ? '#4caf50' : '#ccc', borderRadius: 8 }}>
