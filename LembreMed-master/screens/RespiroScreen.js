...
// Tela de respiração para ansiedade
...
export default function RespiroScreen({ navigation }) {
  ...
  return (
    <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: scheme === 'dark' ? '#222' : '#f7f7f7' }}>
      ...
      <Text style={{ fontSize: fontSize + 2, marginBottom: 16, color: '#222', fontWeight: 'bold' }}>{fase}</Text>
      <Text style={{ fontSize: fontSize + 2, color: '#222', marginBottom: 16 }}>{contador}s</Text>
      <TouchableOpacity style={{ backgroundColor: '#1976d2', padding: 12, borderRadius: 8 }} onPress={() => navigation.goBack()}>
        <Text style={{ color: '#fff', fontSize, fontWeight: 'bold' }}>Voltar</Text>
      </TouchableOpacity>
    </View>
  );
}
