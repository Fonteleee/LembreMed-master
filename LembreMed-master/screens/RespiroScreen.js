...
export default function RespiroScreen({ navigation }) {
  ...
  return (
    <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: scheme === 'dark' ? '#222' : '#fff' }}>
      ...
      <Text style={{ fontSize: fontSize + 4, color: scheme === 'dark' ? '#fff' : '#000', marginBottom: 12 }}>{fase}</Text>
      <Text style={{ fontSize: fontSize + 2, color: scheme === 'dark' ? '#fff' : '#000', marginBottom: 24 }}>{contador > 0 ? contador : ''}</Text>
